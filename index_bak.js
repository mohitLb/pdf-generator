...
async function run() {
  ...
  try {
    // const url = allUrls[i];
    const URL = "https://lakebrains.com/";
    const fileName = URL.split("/")[2]
    let page = await browser.newPage();
    ...
    await page.evaluate(async () => {
      ...
      for (const button of buttons) {
        if (!button.querySelector('a') && (button.innerText.toLowerCase().includes("read") || button.innerText.toLowerCase().includes("more"))) {
          button.click();
        }
      }
    ...
    //   // const elementsWithPageBreak = Array.from(document.body.querySelectorAll('*'))
    //   //   .filter(element => {
    //   //     const styles = window.getComputedStyle(element);
    //   //     // console.log(styles.getPropertyValue('page-break-after'));
    //   //     return styles.getPropertyValue('page-break-after') === 'always';
    //   //   });

    //   // console.log("pageBreak =>", elementsWithPageBreak);
...
const findSectionDivs = async (page, el, index) => {
  if (index < 6) {
    let divtags = await page.$$eval(`${el} > div`, async (els) => {
      let data = []

      async function checkInnerSection(elementName, parentClass) {

        const pagesRequired = Math.ceil((elementName.offsetHeight / 1122) / 2)

        const pagesLength = elementName.childElementCount;

        console.log("page =>", elementName, pagesLength);
        const findSectionDivs2 = async (page, el, index) => {
          const className = el
          if (index < 12) {
            console.log("senasio => 1");
            var data = document.querySelectorAll(`${el}>div`)
            ...
            for (let i = 0; i < data.length; i++) {
              const el = data[i];
              if (el.classList && data[i].childNodes.length >= 3) {
                for (let r = 0; r < data[i].length; r++) {
                  data[i].classList.add(`${parentClass.split(".")[1]}automation_layer2-${i}${r}`)
                }
              }
            ...
            console.log("divtags2 =>", divtags2);

            if (divtags2.length && divtags2.length < 3) {
              console.log("senasio => 1.1");
              for (let i = 0; i < divtags2.length; i++) {
                let element = divtags2[i];
                if (element) {
                  findSectionDivs2(page, element, index + 1);
                } else {
                  findSectionDivs2(page, `${el}>div>div`, index + 1);
                }
              }
            }
            else if (divtags2.length && divtags2.length > 6) {
              console.log("senasio => 1.2");
              let style = "";
              for (let k = 0; k < divtags2.length; k++) {
                const el = divtags2[k];
        ...
        findSectionDivs2([elementName], parentClass, 1)
      }

      for (let i = 0; i < els.length; i++) {
        const el = els[i];
        el.classList.add(`automation_layer1-${i}`)
        let cls = el.getAttribute("class")?.trim()
        if (cls && cls.length) {
          cls = cls.split(" ");
          let parent = ``;
          ...
          data.push(parent)
          if (el.offsetHeight > 1122) {
            await checkInnerSection(el, `.automation_layer1-${i}`)
          }
        }
        else {
          let id = el.getAttribute('id')?.trim()
