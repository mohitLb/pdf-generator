...
var primaryImage = {
  ...
  find: function() {
    ...
    if (userSettingsImage && userSettingsImage.type === 'node') {
    ...
    var findLikelyImage = function(imgs) {
      for (var i = 0; i < imgs.length; i++) {
        ...
        if (!readability.isUnlikelyCandidate(image, 'image') && !readability.hasUnlikelyParent(image)) {
          if (image.nextSibling.tagName === "FIGCAPTION") {
  ...
  metaImage: function() {
    if (pfData.userSettings.primaryImage === 'content-only') { return null; }
    ...
    if (metaSrc) {
      ...
      var $img = $('<img>');
      var attr = pfData.onServer ? 'pf-restore-src' : 'src';
      $img.attr(attr, metaSrc);
      return $img[0];
    } else {
      return null;
    }
    ...
    */
    ...
    return '<div id="pf-src">' +
             '<a id="pf-src-url" href="' + encodedURL + '">' +
               '<img id="pf-src-icon" src="' + pfData.page.favicon + '" >' +
